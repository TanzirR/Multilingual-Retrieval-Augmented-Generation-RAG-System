...
class HybridRetriever:
    ...
    def get_semantic_scores(self, query, top_k=10):
        ...
        for i, chunk_idx in enumerate(I[0]):
            # Skip FAISS "not found" placeholders and non-finite scores
            if chunk_idx == -1:
                continue
            score = float(D[0][i])
            if not np.isfinite(score):
                continue
            semantic_results.append((chunk_idx, score))
        
        return semantic_results
    
    def get_bm25_scores(self, query, top_k=10):
    ...
    def hybrid_retrieve_initial(self, query, initial_k_for_reranking, semantic_weight=0.85, bm25_weight=0.15):
        ...
        for chunk_idx, sem_score in semantic_results:
            ...
            if chunk_idx < 0 or chunk_idx >= len(self.chunks_content):
                continue
            if chunk_idx not in combined_scores:
                combined_scores[chunk_idx] = {'semantic': 0.0, 'bm25': 0.0}
        ...
        for chunk_idx, scores in combined_scores.items():
            ...
            initial_candidates.append({
                'chunk_idx': chunk_idx,
                'chunk_text': self.chunks_content[chunk_idx],
                'metadata': self.chunks_with_metadata[chunk_idx]['metadata'],
    ...
    def retrieve_and_rerank(self, query, initial_k_for_reranking=10, final_k=5, debug=False):
        ...
        if debug:
            # Debug print: Print all initial candidates with their scores
        ...
        if debug:
            ...
            for i, result in enumerate(final_results, 1):
                ...
                chunk_tokens = set(self.tokenize_for_bm25(result['chunk_text']))
...
def answer_question(question, retriever=None, initial_k_for_reranking=10, final_k=5, debug=True):
    ...
    results = retriever.retrieve_and_rerank(question, initial_k_for_reranking, final_k, debug)
    return results

def format_for_llm(question, chunks):
...
Instructions:
Answer questions using ONLY the information provided in the context chunks.
If you do not know the answer, say that you do not know the answer.
Always respond in the same language as the question, regardless of any other text that appears in the prompt formatting.
Ensure your responses are grammatically correct, well-structured, and consist of full, coherent sentences.
Also do not repeat the question in the output."""
